---
sidebar_position: 4
---

# Professional Impact Damages

## SVP Career Damages (Montier Elliott)

- **Lost work time** (20+ hours @ $150/hr): $3,000
- **Career impact** (visibility, credibility): $2,000
- **WiFi disruption** (productivity loss): $1,000
- **Reputation damage**: $3,000

## Total: $9,000

## Why This Matters

Montez was in critical SVP transition period. Forced absences during this period damaged:
- Professional credibility
- Team visibility
- Career advancement momentum
- Potential bonus compensation

See [Tenant Profiles](/docs/01-overview/tenant-profiles) for detailed analysis.
