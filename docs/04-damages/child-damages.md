---
sidebar_position: 5
---

# Child Endangerment Damages

## Health & Safety Damages

- **70+ hours cold exposure**: $3,000
- **Emergency removal necessity**: $2,000

## Disruption & Trauma

- **Routine destruction**: $1,500
- **Holiday disruption**: $1,500
- **Medical consultations**: $500

## Total: $9,000

## Why This Matters

3-year-old endangered by:
- 70+ hours at 57°F (below safe levels)
- No adequate temporary solution
- Emergency removal from home
- Trauma from unsafe conditions
- Holiday season disrupted
- Separation from home during crisis

See [Tenant Profiles](/docs/01-overview/tenant-profiles) for full child impact analysis.
